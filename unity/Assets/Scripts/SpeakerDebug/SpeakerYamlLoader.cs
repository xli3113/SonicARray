...
public class SpeakerYamlLoader : MonoBehaviour {
    ...
    /// <summary>加载并解析，返回解析到的扬声器数量。支持先 try json 再 try yaml。</summary>
    public int Load() {
        speakers.Clear();
        ...
        if (count == 0) {
            string altPath = ResolvePathAlternate();
            if (!string.IsNullOrEmpty(altPath) && altPath != path) {
                speakers.Clear();
                content = File.ReadAllText(altPath);
                count = Parse(content);
                path = count > 0 ? altPath : path;
            }
        }
    ...
    string ResolvePath() {
        var p = Path.Combine(Application.streamingAssetsPath, fileName);
        if (File.Exists(p)) return Path.GetFullPath(p);
        p = Path.Combine(Application.persistentDataPath, fileName);
        if (File.Exists(p)) return Path.GetFullPath(p);
        if (File.Exists(fileName)) return Path.GetFullPath(fileName);
#if UNITY_EDITOR
        var projectRoot = Path.GetDirectoryName(Application.dataPath);
        var cppPaths = new[] {
            Path.Combine(projectRoot, "cpp", fileName),
            Path.Combine(projectRoot, "cpp", "src", fileName),
            Path.Combine(projectRoot, fileName),
        };
        foreach (var cp in cppPaths) {
            if (File.Exists(cp)) return Path.GetFullPath(cp);
        }
#endif
        return null;
    }

    string ResolvePathAlternate() {
        string alt = fileName.EndsWith(".json") ? fileName.Replace(".json", ".yaml") : fileName.Replace(".yaml", ".json").Replace(".yml", ".json");
        var p = Path.Combine(Application.streamingAssetsPath, alt);
        if (File.Exists(p)) return Path.GetFullPath(p);
        p = Path.Combine(Application.persistentDataPath, alt);
        if (File.Exists(p)) return Path.GetFullPath(p);
        return null;
    }
