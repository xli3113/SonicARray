...
**重要**：Unity 的可视化部分**完全独立**于 C++ 后端，可以在没有后端的情况下运行！

### 快速开始（无后端模式）

1. **打开 Unity 项目**
2. **在 `SpatialSource` 组件中**：
   - 取消勾选 `Enable OSC`（禁用 OSC 发送）
   - 这样就不会尝试连接后端，避免错误日志
3. **运行场景（Play 模式）**
4. **在 Game 视图中用鼠标拖拽声源小球**
5. **观察可视化反馈**：
   - 扬声器高亮
   - 连接线显示
   - 扬声器高度变化
...
- ✅ **无错误日志**：不会因为 OSC 连接失败而产生错误
- ✅ **更快启动**：不需要等待后端
- ✅ **专注可视化**：可以专注于调试 Unity 的可视化效果
- ✅ **完全可用**：所有可视化功能都可以正常使用
...
- ✅ 距离计算和增益估算
- ✅ 标签显示
- ✅ 鼠标拖拽交互（`SimpleDrag` 组件）

### 无后端模式无法测试的功能

- ❌ OSC 通信（因为没有后端接收）
- ❌ 真实的 VBAP 算法结果（Unity 使用简化的距离计算）
- ❌ 音频输出（需要后端）
